...
export class ReviewSubmissionService {
  ...
  private readonly commitsSubject = new BehaviorSubject<
    GetProposalReviewCommitResponse[] | null
  >(null);
  ...
  private commits = new Map<string | null, GetProposalReviewCommitResponse>();
  private commitObservables = new Map<
    string | null,
    BehaviorSubject<GetProposalReviewCommitResponse | null>
  >();
  ...
  public async removeCommit(commitSha: string | null): Promise<void> {
    ...
    if (isNotNil(commit.id)) {
      await this.commitReviewApiService.deleteProposalReviewCommit({
        proposalReviewCommitId: commit.id,
      });
    }
  }
  ...
  public async updateCommit(
    ...
    updatedCommit: GetProposalReviewCommitResponse,
  ): Promise<void> {
  ...
  private createCommitObservable(): [
    ...
    const commitSubject =
      new BehaviorSubject<GetProposalReviewCommitResponse | null>(null);
        ...
        batchApiCall(commit => {
          ...
          return from(
            this.commitReviewApiService.updateProposalReviewCommit({
              // [TODO] - we know it's not null here because it's definitely been created already
              proposalReviewCommitId: commit.id!,
