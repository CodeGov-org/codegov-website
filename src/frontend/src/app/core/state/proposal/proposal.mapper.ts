...
export function mapProposalListResponse(
  ...
  return apiResponseList
    .sort(
      (response1, response2) =>
        new Date(response2.proposal.proposed_at).getTime() -
        new Date(response1.proposal.proposed_at).getTime(),
    )
    .map(proposalResponse => ({
      ...
      reviewCompletedAt: getReviewCompletionDate(
        proposalResponse.proposal.review_completed_at,
      ),
      decidedAt: addDays(new Date(proposalResponse.proposal.proposed_at), 3),
      summary:
...
function getReviewCompletionDate(
  reviewCompletedAt: [] | [string],
): Date | undefined {
  return reviewCompletedAt.length ? new Date(reviewCompletedAt[0]) : undefined;
}
